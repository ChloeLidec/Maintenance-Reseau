...
CHOIX=$(whiptail --title "Electron" --menu "Choose what you want to do" 25 80 8 \
"See electron installations" "" \
"Init electron rep" "" 3>&1 1>&2 2>&3)

exitstatus=$?
if [ $exitstatus = 0 ]; then
    if [ "$CHOIX" == "See electron installations" ]; then
    ...
    elif [ "$CHOIX" == "Init electron rep" ]; then
        sudo npm init -y
        sudo npm install --save-dev electron
        echo -e "${GREEN} Electron initialisation done ${COLOR_OFF}"
    fi
else
    echo -e " ${RED} User selected Cancel. ${COLOR_OFF}"
fi