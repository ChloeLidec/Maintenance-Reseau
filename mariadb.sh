...
if [ $exitstatus = 0 ]; then
    if [ $CHOIX = "Install MariaDB" ]; then
        ...
        echo "{GREEN} MariaDB installation done {COLOR_OFF}"
    elif [ $CHOIX = "Configure MariaDB" ]; then
        sudo mysql_secure_installation < .mariadb.txt
        echo "{GREEN} MariaDB configuration done {COLOR_OFF}"
    elif [ $CHOIX = "Create a database" ]; then
        echo "{BLUE} What do you want to name your database? {COLOR_OFF}"
        read DATABASE_NAME
        ...
        if [ $REP = 0 ]; then
            ...
            echo "{BLUE} Choose a user to grant privileges to {COLOR_OFF}"
            read USER_NAME
            echo "GRANT ALL PRIVILEGES ON $DATABASE_NAME.* TO '$USER_NAME'@'localhost' WITH GRANT OPTION;" | sudo mysql
            echo "FLUSH PRIVILEGES;" | sudo mysql
        fi
        echo "{GREEN} Your database has been created! {COLOR_OFF}"
    elif [ $CHOIX = "Create user and grant privileges" ]; then
        echo "{BLUE} What do you want to name your user? {COLOR_OFF}"
        read USER_NAME
        echo "{BLUE} What do you want to name your password? {COLOR_OFF}"
        read USER_PASSWORD
        ...
        echo "FLUSH PRIVILEGES;" | sudo mysql
        echo "{GREEN} Your user has been created! {COLOR_OFF}"
    elif [ $CHOIX = "Show databases" ]; then
        echo "SHOW DATABASES;" | sudo mysql
    elif [ $CHOIX = "Create a table" ]; then
        echo "SHOW DATABASES;" | sudo mysql
        echo "{BLUE} Choose a database to create a table in {COLOR_OFF}"
        read DATABASE_NAME
        echo "USE $DATABASE_NAME;" | sudo mysql
        echo "SHOW TABLES;" | sudo mysql
        echo "{BLUE} What do you want to name your table? {COLOR_OFF}"
        read TABLE_NAME
        echo "{BLUE} How many columns do you want to create? {COLOR_OFF}"
        read COLUMN_NUMBER
        echo "CREATE TABLE $TABLE_NAME (" | sudo mysql
        for ((i=1; i<=$COLUMN_NUMBER; i++))
        do
            echo "{BLUE} What do you want to name your column? {COLOR_OFF}"
            read COLUMN_NAME
            echo "{BLUE} What type of data do you want to store in this column?(Please mind how you write it) {COLOR_OFF}"
            echo "{YELLOW} Examples: VARCHAR(255),INT,DATE,TEXT {COLOR_OFF}"
            read COLUMN_TYPE
            echo "$COLUMN_NAME $COLUMN_TYPE," | sudo mysql
        done
        echo ");" | sudo mysql
        echo "{GREEN} Your table has been created! {COLOR_OFF}"
    elif [ $CHOIX = "Show tables" ]; then
        echo "SHOW DATABASES;" | sudo mysql
        echo "{BLUE} Choose a database to show tables in {COLOR_OFF}"
        read DATABASE_NAME
        echo "USE $DATABASE_NAME;" | sudo mysql
        echo "SHOW TABLES;" | sudo mysql
    fi
